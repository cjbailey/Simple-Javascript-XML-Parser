...
XmlDocument.prototype = Object.assign( Object.create( XmlElement.prototype ), {
	toString( args ) {
		...
		let newLine = args.prettyPrint ? "\n" : "";
		return this._children.length > 0 ? ( this._attributes.length > 0 ? `<?xml ${this._attributes.join(" ")}?>${newLine}` : "" ) + this._children[ 0 ].toString( args ).trim().replace( /\n\n+/g, "\n" ) : "";
	},
